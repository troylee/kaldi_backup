...
int main(int argc, char *argv[]) {
  try {
    ...
    std::string gmm_model_rxfilename = po.GetArg(1), noisy_feature_rspecifier =
        po.GetArg(2),
        noiseparams_rspecifier = po.GetArg(3), clean_feature_wspecifier = po
            .GetArg(4);
    ...
    SequentialBaseFloatMatrixReader noisy_feature_reader(
        noisy_feature_rspecifier);
