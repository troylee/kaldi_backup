...
for x in train_si84_clean train_si84_multi ; do  
  $local/flist2scp_wSuffix.pl $x.flist | sort > ${x}_sph_tmp.scp
  cat ${x}_sph_tmp.scp | awk '{print $1}' \
    | $local/find_transcripts.pl dot_files.flist > ${x}_tmp.trans1
  cat ${x}_sph_tmp.scp | awk '{printf("%s.0 %s\n", $1, $2);}' > ${x}_sph.scp
  cat ${x}_tmp.trans1 | awk '{printf("%s.0 ", $1); for(i=2;i<=NF;i++) printf("%s ", $i); printf("\n");}' > ${x}.trans1
done

# Trans and sph for Dev Set
for x in $(seq -f "%02g" 01 14); do
  $local/flist2scp_wSuffix.pl dev_0330_${x}.flist | sort > dev_0330_${x}_sph_tmp.scp
  $local/flist2scp_wSuffix.pl dev_1206_${x}.flist | sort > dev_1206_${x}_sph_tmp.scp
...
for x in $(seq -f "%02g" 01 14); do
  $local/flist2scp_wSuffix.pl test_eval92_${x}.flist | sort > test_eval92_${x}_sph_tmp.scp
...
noiseword="<NOISE>";
for x in train_si84_clean train_si84_multi test_eval92 dev_0330 dev_1206; do
...
# use 5k language models only
cp $CORPUS/wsj0/doc/lng_modl/base_lm/bcb05onp.z  $lmdir/lm_bg.arpa.gz || exit 1;
chmod u+w $lmdir/lm_bg.arpa.gz

# trigram would be: !only closed vocabulary here!
cp $CORPUS/wsj0/doc/lng_modl/base_lm/tcb05cnp.z $lmdir/lm_tg.arpa.gz || exit 1;
