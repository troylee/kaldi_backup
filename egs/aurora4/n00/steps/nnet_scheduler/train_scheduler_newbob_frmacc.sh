...
acc=$(cat $dir/log/iter00.initial.log | grep "FRAME_ACCURACY" | tail -n 1 | awk '{ print $3; }' | sed "s:%::g")
echo "CROSSVAL PRERUN FRAME.ACC $(printf "%.4f" $acc)"
...
for iter in $(seq -w $max_iters); do
  ...
  tr_acc=$(cat $dir/log/iter${iter}.tr.log | grep "FRAME_ACCURACY" | tail -n 1 | awk '{ print $3; }' | sed "s:%::g")
  echo -n "TRAIN FRAME.ACC $(printf "%.4f" $tr_acc), (lrate$(printf "%.6g" $learn_rate)), "
  ...
  acc_new=$(cat $dir/log/iter${iter}.cv.log | grep "FRAME_ACCURACY" | tail -n 1 | awk '{ print $3; }' | sed "s:%::g")
  echo -n "CROSSVAL FRAME.ACC $(printf "%.4f" $acc_new), "
  ...
  # stopping criterion
   if [[ "1" == "$(awk "BEGIN{print($learn_rate < $end_learn_rate)}")" ]]; then
    ...
    echo finished, too small rel. improvement $(awk "BEGIN{print($acc_prev-$acc)}")
    break
  fi
